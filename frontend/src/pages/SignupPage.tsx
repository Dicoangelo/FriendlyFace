...
export default function SignupPage() {
  ...
  const strengthColors = ["", "bg-rose-ember", "bg-gold", "bg-teal"];
  ...
  return (
    <div className="min-h-screen bg-page flex items-center justify-center px-4 relative overflow-hidden">
      {/* Background orbs */}
      <div className="absolute top-1/3 -right-20 w-72 h-72 bg-magenta/10 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-1/3 -left-20 w-80 h-80 bg-cyan/10 rounded-full blur-[100px] pointer-events-none" />

      <div className="w-full max-w-md relative">
        <div className="text-center mb-8">
          <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-magenta to-cyan p-0.5">
        ...
        >
                  ...
                  {[1, 2, 3].map((level) => (
                    <div
                      ...
                      className={`h-1 flex-1 rounded-full transition-colors ${
                        passwordStrength >= level ? strengthColors[passwordStrength] : "bg-surface-light"
                      }`}
                    />
                  ))}
                </div>
                <span className={`text-[10px] font-medium ${
                  passwordStrength === 1 ? "text-rose-ember" : passwordStrength === 2 ? "text-gold" : "text-teal"
                }`}>
          ...
          <p className="text-center text-sm text-fg-muted">
            ...
            <Link to="/login" className="text-cyan hover:text-cyan-dim font-medium">
              Sign in
            </Link>
          </p>
        </form>
