...
\item \textbf{Open-source reference implementation} with 1,415 tests (93\% coverage) and a production deployment.
\end{enumerate}
...
\textbf{React Frontend (L6)} --- Dashboard, Consent UI, Audit Views \\[2pt]
\textbf{FastAPI REST Layer} --- 84+ endpoints, SSE, Auth, RBAC \\[2pt]
\begin{tabular}{|c|c|c|c|c|}
\hline
L1 Recog. & L2 FL & L4 Fair. & L5 XAI & L6 Gov. \\
PCA+SVM & FedAvg & DP+EO & LIME & Consent \\
Voice & DP-Fed & Auto & SHAP & Comply \\
 & Poison & Audit & SDD & OSCAL \\
\hline
\end{tabular} \\[2pt]
\textbf{L3: Blockchain Forensic Layer} \\
ForensicEvent $\to$ Hash Chain $\to$ Merkle Tree $\to$ Prov.\ DAG \\
Schnorr ZK Proofs $\cdot$ Ed25519 DID:key $\cdot$ VCs $\cdot$ ForensicSeal \\[2pt]
\textbf{Storage:} SQLite (default) $|$ Supabase (cloud)
}}
\caption{FriendlyFace six-layer architecture with ForensicSeal.}
...
\textbf{Continuous Compliance.} Seals carry expiry timestamps (default 365 days). Renewal triggers fresh six-layer compliance checks. Seal issuance completes in 3.3\,ms.
...
FriendlyFace includes 1,415 test functions across 40+ test files, achieving 93\% line coverage.
